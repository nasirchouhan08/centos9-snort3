...
cd ~/sources/snort3 && ./~/sources/snort3/configure_cmake.sh --prefix=/usr/local/snort --enable-tcmalloc
cd ~/sources/snort3/build && sudo make -j$(nproc) && sudo make -j$(nproc) install
